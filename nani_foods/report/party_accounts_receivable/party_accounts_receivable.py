	...
	def run(self, args):
		...
		for row in self.data:
			if self.filters.get('due_date'):
				if datetime.strptime(self.filters.get('due_date'), '%Y-%m-%d') > datetime.strptime(str(row.due_date), '%Y-%m-%d'):
					due_date_data.append(row)
		if self.filters.get('outstanding_amount') == 1:
			if self.filters.get('due_date'):
				for row in due_date_data:
					if row.outstanding != None:
				...
				for row in self.data:
                                        if row.outstanding != None:
                                                if row.outstanding > 0:
                                                        main_data.append(row)
		if self.filters.due_date and not self.filters.outstanding_amount:
			return self.columns, due_date_data, None, self.chart, None, self.skip_total_row
		if not self.filters.due_date and self.filters.outstanding_amount:
			return self.columns, main_data, None, self.chart, None, self.skip_total_row
		if self.filters.due_date and self.filters.outstanding_amount:
			return self.columns, main_data, None, self.chart, None, self.skip_total_row
		else:
